...
function DataProvider({ children, settings }) {
  ...
  const _loadData = async () => {
    const jStrings = await utils.file.loadJSON(
      `${import.meta.env.BASE_URL}data/strings.json`
    );
    const jProfile = await utils.file.loadJSON(
      `${import.meta.env.BASE_URL}data/profile.json`
    );
    const jCategories = await utils.file.loadJSON(
      `${import.meta.env.BASE_URL}data/categories.json`
    );
    const jSections = await utils.file.loadJSON(
      `${import.meta.env.BASE_URL}data/sections.json`
    );
